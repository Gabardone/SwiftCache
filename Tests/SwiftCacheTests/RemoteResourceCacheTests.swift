...
final class SwiftCacheTests: XCTestCase {
    ...
    func testLocallyStoredImageDataHappyPath() async throws {
        ...
        let inMemoryExpectation = expectation(description: "No in memory data found")
        imageCache.inMemoryStorage.storedValueForOverride = { url in
            XCTAssertEqual(url, Self.dummyURL)
            inMemoryExpectation.fulfill()
            return nil
        }

        let image = try await imageCache.cache.fetchResourceWith(identifier: Self.dummyURL)

        await fulfillment(of: [localDataExpectation, inMemoryExpectation])
